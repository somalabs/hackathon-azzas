"""
Análise de Similaridade INV26 v3 — Fábula
Correções vs v2:
  - ON_DAYS ajustado por coleção: INV23=122 (Jun-Set), INV24=214 (Mar-Set), INV25=206 (Mar-Set)
  - Fator de crescimento: x1.111 (média INV23->24 +9.9% e INV24->25 +12.4% via Barra Shopping)
  - Threshold demanda_arredondada: >= 0.3 -> max(1, round) | antes era > 0.5
...
# INV26 precos (coletados inline da query BQ — sem dados pos 31/03/2026)
precos_inv26_raw = [
...
# ─── 2. Funcoes de feature engineering ──────────────────────────────────────
...
# ─── CORRECAO v3: ON_DAYS ajustado por colecao ──────────────────────────────
# INV23: Jun 01 - Set 30, 2023 = 122 dias (mantido — colecao ainda era junio)
# INV24: Mar 01 - Set 30, 2024 = 214 dias (colecao migrou para marco)
# INV25: Mar 08 - Set 30, 2025 = 206 dias (lancamento confirmado 08/03)
ON_DAYS  = {"INV23": 122, "INV24": 214, "INV25": 206}
PESO_COL = {"INV23": 1.0, "INV24": 2.0, "INV25": 3.0}

# Fator de crescimento inter-colecao aplicado sobre INV25 para projetar INV26
# Media de: INV23->INV24 +9.9% e INV24->INV25 +12.4% (Barra Shopping como referencia)
GROWTH_FACTOR = 1.111
...
# Aplicar fator de crescimento inter-colecao para projetar INV26 a partir de INV25
df_vel["demanda_prevista"] = df_vel["velocidade_prevista"] * df_vel["horizonte_dias"] * GROWTH_FACTOR

# Threshold 0.3: qualquer demanda >= 0.3 recebe ao menos 1 unidade
df_vel["demanda_arredondada"] = df_vel["demanda_prevista"].apply(
    lambda x: max(1, round(x)) if x >= 0.3 else 0
)

print("\n=== RESULTADO v3 ===")
print("Metodo de calculo:")
print(df_vel["metodo"].value_counts().to_string())
print(f"\nVelocidade mediana: {df_vel['velocidade_prevista'].median():.4f} pcs/dia")
print(f"\nDemanda por filial (horizonte alvo, pos-crescimento):")
...
print(f"\nDemanda total INV26 prevista: {df_vel['demanda_arredondada'].sum()} pecas")
print(f"Zeros (nao envia): {(df_vel['demanda_arredondada'] == 0).sum()}")
print(f"Com demanda: {(df_vel['demanda_arredondada'] > 0).sum()}")

out = BASE / "velocidade_prevista_inv26_v3.csv"
df_vel.to_csv(out, index=False)
print(f"\nSalvo: {out}")

# Comparacao v2 vs v3
v2 = pd.read_csv(BASE / "velocidade_prevista_inv26_v2.csv")
comp = (
    df_vel.groupby("FILIAL")["demanda_arredondada"].sum()
    .rename("v3")
    .to_frame()
    .join(v2.groupby("FILIAL")["demanda_arredondada"].sum().rename("v2"))
)
comp["delta_pct"] = ((comp["v3"] - comp["v2"]) / comp["v2"].replace(0, np.nan) * 100).round(1)
print("\n=== Comparacao v2 -> v3 (demanda total por filial) ===")
print(comp.sort_values("v3", ascending=False).to_string())
