"""
Analise de Similaridade INV26 v4 - Fabula
Correcoes vs v3:
  - Calibracao por loja: ancora a velocidade media ao INV25 real x GROWTH_FACTOR
  - Preserva diferenciacao relativa entre produtos do modelo KNN
  - Corrige subestimacao de v3 causada pelo ajuste de ON_DAYS (122->206/214 dias)

Logica de calibracao:
  calib_fator[loja] = (vel_INV25_media[loja] * GROWTH_FACTOR) / vel_v3_media[loja]
  vel_calibrada = vel_v3 * calib_fator  -> media da loja = INV25 * crescimento
  demanda = vel_calibrada * horizonte   -> growth ja embutido, nao reaplicar

Data-base: 31/03/2026  |  Canal: Varejo fisico  |  Marca: Fabula
"""
...
# INV26 precos (query BQ - sem dados pos 31/03/2026)
precos_inv26_raw = [
    ...
    ("5.20953",198,99,42.15),("5.20926",149,74.5,24.03),("5.21666",198,198,43.4),
...
print(f"Precos INV26 carregados: {len(precos_inv26)} produtos")
...
def parse_preco_tier(preco):
    ...
    if preco <  100: return 1
    if preco <  200: return 2
    if preco <  300: return 3
    return 4

def parse_is_beach(linha, grupo):
    l = str(linha).upper() if pd.notna(linha) else ""
    g = str(grupo).upper() if pd.notna(grupo) else ""
    return 1 if ("PRAIA" in l or any(k in g for k in ["BIQUINI","SUNGA","MAIAO","UV"])) else 0
...
inv26 = enrich(inv26)
print(f"INV26 enriquecido: {len(inv26)} produto-cor")

# --- 4. Enriquecer historico --------------------------------------------------
...
# ON_DAYS por colecao (v3 correcao mantida):
# INV23: Jun-Set 2023 = 122 dias | INV24: Mar-Set 2024 = 214 dias | INV25: Mar-Set 2025 = 206 dias
ON_DAYS  = {"INV23": 122, "INV24": 214, "INV25": 206}
PESO_COL = {"INV23": 1.0, "INV24": 2.0, "INV25": 3.0}
GROWTH_FACTOR = 1.111  # media de +9.9% (INV23->24) e +12.4% (INV24->25) via Barra Shopping
...
print(f"Vel. historicas calculadas: {len(vel_hist)} linhas")
...
def similarity(a, b):
    ...
    bea = int(a.get("is_beach", 0) or 0)
    beb = int(b.get("is_beach", 0) or 0)
    if bea == beb:
        score += WEIGHTS["is_beach"]
    iba = int(a.get("is_bebe", 0) or 0)
    ibb = int(b.get("is_bebe", 0) or 0)
    if iba == ibb:
        score += WEIGHTS["is_bebe"]
    return score

# --- 6. Calcular velocidade prevista (KNN) ------------------------------------
...
results = []
n_total = len(inv26)
print(f"\nProcessando {n_total} produto-cor x {len(TARGET_STORES)} filiais...")
...
# --- 7. CALIBRACAO v4: ancora ao INV25 real x crescimento por loja -----------
# Corrige subestimacao gerada pelo ajuste de ON_DAYS:
#   calib_fator = (vel_INV25_media * GROWTH_FACTOR) / vel_v3_media
#   vel_calibrada = vel_v3 * calib_fator  (media loja = INV25 * crescimento)
#   demanda = vel_calibrada * horizonte   (growth ja embutido, nao reaplicar)

h25 = hist10[hist10["COLECAO"] == "INV25"].copy()
h25_agg = (
    h25.groupby("filial_canon")
    .agg(pecs=("pecas_vendidas", "sum"), n_skus=("PRODUTO", "count"))
    .reset_index()
)
h25_agg["vel_inv25_media"] = (
    h25_agg["pecs"].clip(lower=0) / ON_DAYS["INV25"] / h25_agg["n_skus"]
)

vel_v3_media_loja = (
    df_vel.groupby("FILIAL")["velocidade_prevista"]
    .mean()
    .reset_index()
    .rename(columns={"velocidade_prevista": "vel_v3_media"})
)

calib_df = (
    h25_agg.rename(columns={"filial_canon": "FILIAL"})
    .merge(vel_v3_media_loja, on="FILIAL", how="inner")
)
calib_df["calib_fator"] = (
    calib_df["vel_inv25_media"] * GROWTH_FACTOR / calib_df["vel_v3_media"]
)
calib_map = dict(zip(calib_df["FILIAL"], calib_df["calib_fator"]))

print("\nFatores de calibracao v4 por loja:")
for loja, f in sorted(calib_map.items(), key=lambda x: -x[1]):
    print(f"  {loja.replace('FABULA ',''):22s}: x{f:.3f}")

df_vel["calib_fator"]        = df_vel["FILIAL"].map(calib_map).fillna(1.0)
df_vel["velocidade_calibrada"] = df_vel["velocidade_prevista"] * df_vel["calib_fator"]

# --- 8. Metricas & output -----------------------------------------------------
df_vel = df_vel.merge(leadtimes, on="FILIAL", how="left")
df_vel["horizonte_dias"] = 14 + df_vel["LEADTIME"]

# Growth ja embutido na calibracao: demanda = vel_calibrada * horizonte
df_vel["demanda_prevista"]    = df_vel["velocidade_calibrada"] * df_vel["horizonte_dias"]
df_vel["demanda_arredondada"] = df_vel["demanda_prevista"].apply(
    lambda x: max(1, round(x)) if x >= 0.3 else 0
)

print("\n=== RESULTADO v4 (calibrado) ===")
print("Metodo KNN:")
print(df_vel["metodo"].value_counts().to_string())
print(f"\nVelocidade calibrada mediana: {df_vel['velocidade_calibrada'].median():.4f} pcs/dia")
print(f"\nDemanda por filial (horizonte alvo, calibrado ao INV25 real):")
resumo = df_vel.groupby("FILIAL").agg(
    demanda_total   = ("demanda_arredondada",  "sum"),
    horizonte_dias  = ("horizonte_dias",       "first"),
    vel_calibrada   = ("velocidade_calibrada", "mean"),
    calib_fator     = ("calib_fator",          "first"),
).sort_values("demanda_total", ascending=False)
resumo["vel_calibrada"] = resumo["vel_calibrada"].round(4)
resumo["calib_fator"]   = resumo["calib_fator"].round(3)
print(resumo.to_string())
print(f"\nDemanda total INV26 v4: {df_vel['demanda_arredondada'].sum()} pecas")
print(f"Zeros (nao envia):      {(df_vel['demanda_arredondada'] == 0).sum()}")
print(f"Com demanda:            {(df_vel['demanda_arredondada'] > 0).sum()}")

out = BASE / "velocidade_prevista_inv26_v4.csv"
df_vel.to_csv(out, index=False)
print(f"\nSalvo: {out}")

# Comparacao v3 -> v4
v3 = pd.read_csv(BASE / "velocidade_prevista_inv26_v3.csv")
comp = (
    df_vel.groupby("FILIAL")["demanda_arredondada"].sum()
    .rename("v4")
    .to_frame()
    .join(v3.groupby("FILIAL")["demanda_arredondada"].sum().rename("v3"))
)
comp["delta_pct"] = ((comp["v4"] - comp["v3"]) / comp["v3"].replace(0, np.nan) * 100).round(1)
print("\n=== Comparacao v3 -> v4 (demanda total por filial) ===")
print(comp.sort_values("v4", ascending=False).to_string())
